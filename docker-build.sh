...
echo 
echo "/######################################################################/"
echo " Build new image terrabrasilis/fires-data-task:v$VERSION "
echo "/######################################################################/"
echo

docker build $NO_CACHE -t "terrabrasilis/fires-data-task:v$VERSION" --build-arg VERSION="v$VERSION" -f env-scripts/Dockerfile .
...
if [[ ! "$SEND_TO_HUB" = "yes" ]]; then
    ...
    docker push "terrabrasilis/fires-data-task:v$VERSION"
fi